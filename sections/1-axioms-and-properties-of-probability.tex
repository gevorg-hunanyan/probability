...
\begin{enumerate}[series=prob]
    ...
    \prob{shootingthreetimes}{
        Consider the experiment of shooting at a target three times. Let $A_i$ be the event of $i$-th bullet hitting the target, and let $\PP(A_i)=P_i$. Find the probabilities of the following events:
        \begin{enumerate}
    ...
    \prob{geom1}{Let $A=\{(x,y)\in\Omega\;\vert\; x+y\le1\}$ and $B=\{(x,y)\in\Omega\;\vert\;y\le x^2\}$ be events of the sample space $\Omega=[0,1]\times[0,1]$. Draw on the plane the events $A\cup B$, $A\cap B$, $A\setminus B$ $\overline{A}\cap\overline{B}$. Find the probabilities of each of these events in the geometric probability model.}
    \prob{polynom1}{
    ...
    \prob{geometric2d}{Two points are randomly chosen in $[0,1]$. Find the probability that the distance between those points is less than $\alpha\in(0,1)$. Use the geometric probability model.}
    \prob{polynom2}{
        Find the probability that the roots of the equation $x^2+bx+c=0$ are
        \begin{enumerate}
            \item real,
            \item positive,
        \end{enumerate}
        if the coefficients $0\le b\le 1$ and $0\le c\le 1$ are chosen randomly. Use the geometric probability model.
    }
    \prob{polynom3}{
    ...
    \prob{stick-break-triangle}{
        A stick of length 1 is broken at two random points to form three pieces. Find the probability that those three pieces can form a triangle. Use the geometric probability model.
    }
    \prob{continuity}{
    ...
    \prob{five-envelopes}{
        There are 5 different letters and 5 different envelopes. Each letter is placed in an envelope at random. What is the probability that no letter is placed in its correct envelope?
    }
    \prob{polynom4}{
        Find the probability of the roots of the equation $ax^2+bx+c=0$ being real if the coefficients are chosen randomly in the cube $0<a\le1$, $0<b\le1$, $0<c\le1$. Use the geometric probability model.
    }
    \prob{nonmeasurable}{
        Prove that it is not possible to define a translation-invariant probability $\PP$ on the experiment $([0,1),2^{[0,1)})$ such that the probability of any interval is its length. Translation invariance means that for any $x\in\RR$ and any $A\in2^{[0,1)}$, we have
        \[
            \PP(A)=\PP(\{a+x\mod1\mid a\in A\}).
        \]
    }
\end{enumerate}